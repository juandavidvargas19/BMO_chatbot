...
def create_rag_agent(_llm, _retriever):
    ...
    def retriever_tool(query: str) -> str:
        ...
        print(_retriever, "RETRIEVER IN LANGG")
...
def create_rag_agent_v2(_llm, _retriever, _llm_evaluator):
    """
    Create RAG agent with evaluation step and self-refinement loop.
    ...
    Returns:
        Compiled RAG agent graph with evaluation and refinement
    """
    print("🤖 Creating improved RAG agent with self-refinement...")
    ...
    class AgentState(TypedDict):
        ...
        refinement_attempts: int  # Track number of refinement attempts
        previous_responses: List[str]  # Store previous responses for context
        evaluation_feedback: str  # Store detailed evaluation feedback
    ...
    def should_refine(state: AgentState):
        """Check if response needs refinement based on evaluation score."""
        evaluation_score = state.get('evaluation_score', 0.0)
        refinement_attempts = state.get('refinement_attempts', 0)
        
        # Refine if score < 0.5 and haven't exceeded max attempts
        needs_refinement = evaluation_score < 0.5 and refinement_attempts < 2
        
        print(f"🔍 Evaluation check: Score={evaluation_score:.3f}, Attempts={refinement_attempts}, Needs refinement={needs_refinement}")
    ...
    IMPORTANT CONTEXT: You are REFINING a previous response that received a low quality score.
    
    Previous Response (Score: {previous_score:.3f}/1.0):
    {previous_response}
    ...
    1. Address the specific issues mentioned in the evaluation feedback
    2. Improve relevance, accuracy, completeness, clarity, and citations
    3. Use the retriever tool to gather additional relevant information if needed
    4. Provide more specific examples and detailed explanations
    5. Ensure all claims are well-supported with proper citations
    6. Structure your response clearly and professionally
    
    Please provide an IMPROVED response to the original question:
    {original_query}
    """
    
    evaluation_prompt = """
    You are a top 1 percent expert evaluator tasked with assessing top notch scientists. 
    Be critical and demanding in your evaluation as your reputation as a top expert is on the line.
    ...
    - Clarity: Is the response clear and well-structured so as to be understood by any professional and has zero ambiguity?          
    ...
    Write the scores for each component with brief explanations. Then write the sum of all (X.XX, e.g. 0.48).      

    IMPROVEMENT SUGGESTIONS (if score < 0.5):
    Provide specific suggestions for improving the response:
    - What information is missing?
    - How can citations be improved?
    - What examples should be added?
    - How can clarity be enhanced?

    IMPORTANT: After your analysis, you MUST end your response with exactly this format:
    <<<SCORE>>>X.XX<<<END>>>
    """
    ...
    def call_llm(state: AgentState) -> AgentState:
        ...
        if refinement_attempts > 0:
            # This is a refinement - use refinement prompt with context
            previous_responses = state.get('previous_responses', [])
            evaluation_feedback = state.get('evaluation_feedback', '')
            previous_score = state.get('evaluation_score', 0.0)
    ...
    def evaluate_response(state: AgentState) -> AgentState:
        ...
        # Store previous responses for refinement context
        previous_responses = state.get('previous_responses', [])
        ...
        return {
            ...
            'evaluation_feedback': response_text
        }
    
    def prepare_refinement(state: AgentState) -> AgentState:
        """Prepare for refinement by incrementing attempt counter and resetting messages."""
        refinement_attempts = state.get('refinement_attempts', 0) + 1
        
        print(f"🔄 Preparing refinement attempt {refinement_attempts}/2 (Score: {state.get('evaluation_score', 0.0):.3f})")
        
        # Reset messages to original query for refinement
        original_query = state['user_query']
        new_messages = [HumanMessage(content=original_query)]
    ...
    graph.add_node("prepare_refinement", prepare_refinement)
    ...
    # After evaluation, check if refinement is needed
    graph.add_conditional_edges(
        "evaluator",
        should_refine,
        {True: "prepare_refinement", False: END}
    )
    
    # After preparing refinement, go back to LLM
    graph.add_edge("prepare_refinement", "llm")
    ...
    print("✅ RAG agent with self-refinement created successfully!")
    return rag_agent
    ...
    def call_llm(state: AgentState) -> AgentState:
        ...
        if 'conversation_history' in state and state['conversation_history']:
            ...
            for i, conv in enumerate(recent_conversations, 1):
                ...
                history_context += f"Previous A{i}: {conv['assistant_response'][:200]}...\n\n"
...
@sl.cache_resource
def create_rag_agent_v4(_llm, _retriever, _llm_evaluator):
    """
    Create RAG agent with evaluation step, self-refinement loop, and conversation memory.
    ...
    Returns:
        Compiled RAG agent graph with evaluation, refinement, and memory
    """
    print("🤖 Creating RAG agent v4 with conversation memory and self-refinement...")
    ...
    def call_llm(state: AgentState) -> AgentState:
        ...
        if refinement_attempts > 0:
            ...
            if conversation_history:
                conversation_context = "\n\nPrevious conversation context (for reference):\n"
                recent_conversations = conversation_history[-2:]
                for i, conv in enumerate(recent_conversations, 1):
                    conversation_context += f"Previous Q{i}: {conv['user_query']}\n"
                    conversation_context += f"Previous A{i}: {conv['assistant_response'][:200]}...\n\n"
            ...
            if previous_responses:
                ...
                if conversation_context:
                    context_prompt = f"{system_prompt}{conversation_context}"
                prompt = context_prompt
        else:
            # First attempt - use standard prompt with conversation context
            context_prompt = system_prompt
    ...
    try:
        graph_image = rag_agent.get_graph().draw_mermaid_png()
        with open("rag_agent_graph_v4.png", "wb") as f:
            f.write(graph_image)
        print("✅ RAG agent v4 graph saved as 'rag_agent_graph_v4.png'")
    except Exception as e:
        print(f"⚠️ Could not save RAG agent graph: {e}")
    
    print("✅ RAG agent v4 created successfully!")
    return rag_agent
