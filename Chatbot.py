...
from Langgraph_Agent import initialize_llm_and_embeddings_v2, create_rag_agent_v2, create_rag_agent_v3, create_rag_agent_v4
import argparse
import uuid
import tempfile
import shutil
...
def process_uploaded_pdf(uploaded_file, embeddings, version):
    """Process uploaded PDF file and create new RAG components"""
    ...
    try:
        ...
        pages_split = load_and_process_pdf(temp_pdf_path)
        ...
        return vectorstore, retriever, pages_split, f"✅ Successfully processed: {uploaded_file.name}"
        ...
        if 'temp_pdf_path' in locals():
            try:
                ...
                pass
        return None, None, None, f"❌ Error processing {uploaded_file.name}: {str(e)}"
...
def main():
    ...
    parser.add_argument("--architecture_version", "-v", type=int, choices=[1,2,3,4])
    ...
    # Sidebar for PDF selection and system info
    with sl.sidebar:
        sl.header("📄 Document Selection")
        
        # PDF source selection
        pdf_source = sl.radio(
            "Choose PDF source:",
            ["📁 Default PDF (Machine Learning)", "📤 Upload Custom PDF"],
            index=0
        )
        
        # Initialize session state for PDF processing
        if 'pdf_processed' not in sl.session_state:
            sl.session_state.pdf_processed = False
            sl.session_state.current_pdf_source = None
            sl.session_state.vectorstore = None
            sl.session_state.retriever = None
        ...
        if pdf_source == "📤 Upload Custom PDF":
            ...
            if uploaded_file is not None:
                sl.success(f"📎 File selected: {uploaded_file.name}")
                sl.info(f"📊 Size: {uploaded_file.size / 1024:.1f} KB")
                
                # Process button for uploaded file
                if sl.button("🚀 Process Uploaded PDF", type="primary"):
                    with sl.spinner("🔄 Processing uploaded PDF..."):
                        try:
                            vectorstore, retriever, pages_split, status_msg = process_uploaded_pdf(
                                uploaded_file, embeddings, version
                            )
                            
                            if vectorstore is not None:
                                # Store in session state
                                sl.session_state.vectorstore = vectorstore
                                sl.session_state.retriever = retriever
                                sl.session_state.pdf_processed = True
                                sl.session_state.current_pdf_source = "uploaded"
                                sl.success(status_msg)
                                sl.rerun()
                            else:
                                sl.error(status_msg)
                                
                        except Exception as e:
                            sl.error(f"❌ Error processing uploaded PDF: {e}")
        else:
            # Default PDF option
            sl.info("📖 Using default: random_machine_learning.pdf")  # Fixed filename
            
            # Process button for default file
            if sl.button("🚀 Process Default PDF", type="primary"):
                with sl.spinner("🔄 Processing default PDF..."):
                    try:
                        pdf_path = "random_machine_learning.pdf"  
                        
                        # Check if file exists
                        if not os.path.exists(pdf_path):
                            sl.error(f"❌ Default PDF file not found: {pdf_path}")
                        else:
                            ...
                            sl.session_state.retriever = retriever
                            sl.session_state.pdf_processed = True
                            sl.session_state.current_pdf_source = "default"
                            
                            sl.success("✅ Default PDF processed successfully!")
                            sl.rerun()
                            
                    except Exception as e:
                        sl.error(f"❌ Error processing default PDF: {e}")
                        print(f"Debug - Error details: {e}")  # Add debug info

        sl.markdown("---")
        
        # Show current status
        if sl.session_state.get('pdf_processed', False):
            sl.success(f"✅ PDF Ready: {sl.session_state.current_pdf_source}")
        else:
            sl.warning("⚠️ No PDF processed yet")

    # Main content area
    sl.header("🧠 PDF RAG Assistant")
    version_descriptions = {
        1: "🤖 Basic RAG Agent",
        2: "🤖 RAG Agent with quality check", 
        3: "🤖 RAG Agent with Conversation Memory",
        4: "🤖 RAG Agent with quality-check + Conversation Memory"
    }
    sl.markdown(f"**{version_descriptions.get(version, '🤖')} Version {version}** - Ask questions about PDF documents with comprehensive metrics tracking")

    # Get components from session state
    vectorstore = sl.session_state.get('vectorstore')
    retriever = sl.session_state.get('retriever')
    
    print(f"Debug - Retriever status: {retriever is not None}")
    print(f"Debug - PDF processed: {sl.session_state.get('pdf_processed', False)}")
    
    # Create RAG agent only if retriever is available
    rag_agent = None
    if retriever is not None:
        try:
            if version == 1:
                ...
                rag_agent = create_rag_agent_v4(llm, retriever, llm_evaluator)
            print("✅ RAG agent created successfully")
        except Exception as e:
            sl.error(f"❌ Error creating RAG agent: {e}")
            print(f"Debug - RAG agent error: {e}")
    else:
        if not sl.session_state.get('pdf_processed', False):
            sl.info("👆 Please process a PDF document first using the sidebar")
        else:
            sl.error("❌ Retriever not available despite PDF being processed")
    ...
    # Initialize conversation memory for version 3 and 4
    if version > 2 and 'conversation_memory' not in sl.session_state:
        sl.session_state.conversation_memory = []  # Store last 3 conversations
    
    # Chat interface
    sl.markdown("---")
    sl.subheader("💬 Ask Questions")
    
    query = sl.text_input(
        'Enter your question about the document:',
        placeholder="What would you like to know about this PDF?",
        help="Ask any question about the content of the processed PDF document"
    )
    
    # Display conversation memory for version 3 and 4
    if version > 2 and sl.session_state.get('conversation_memory'):
    ...
    if (query and 
        ...
        query != sl.session_state.last_processed_query and
        rag_agent is not None):
        
        # Initialize trackers
        tracker = EmissionsTracker(
            project_name="streamlit_pdf_chatbot",
        ...
        with sl.spinner("🤔 Processing your question..."):
            ...
            # Store conversation in memory (version 3 and 4)
            if version > 2:
